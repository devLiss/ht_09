...
export const jwtService = {
    ...
    async generateTokens(user:any){
        const token = jwt.sign({userId:user.id.toString()}, settings.JWT_SECRET, {expiresIn:'10s'})
        const refreshToken = jwt.sign({userId:user.id.toString()}, settings.JWT_REFRESH_SECRET, {expiresIn:'20s'})
